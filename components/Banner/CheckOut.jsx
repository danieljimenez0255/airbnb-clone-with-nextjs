import { XCircleIcon } from "@heroicons/react/solid";
import format from "date-fns/format";
import { useRecoilState, useRecoilValue } from "recoil";
import {
  checkInDate,
  checkOutDate,
  locationAtom,
  searchClickState,
  searchHoverState,
} from "@/lib/atoms";

const CheckOut = () => {
  const [clickState, setClickState] = useRecoilState(searchClickState);
  const [hoverState, setHoverState] = useRecoilState(searchHoverState);
  const [startDate, setStartDate] = useRecoilState(checkInDate);
  const [endDate, setEndDate] = useRecoilState(checkOutDate);
  const searchInput = useRecoilValue(locationAtom);

  return (
    <>
      <div
        className={`py-[14px] px-6 flex-grow  rounded-[32px] cursor-pointer  ${
          clickState !== 3 || !clickState === 1
            ? "hover:bg-[#EBEBEB]"
            : "bg-white search__shadow border border-black"
        } ${
          endDate !== null && endDate < startDate
            ? "border-2 border-red-600 bg-red-600 opacity-70"
            : ""
        }`}
        onMouseEnter={() => setHoverState(3)}
        onMouseLeave={() => setHoverState(3)}
        onClick={() => {
          if (startDate === null && searchInput[0] !== "") {
            alert("Select check in date!");
            if (clickState !== 2) {
              setClickState(2);
            }
          } else if (startDate === null && searchInput[0] == "") {
            alert("You need to first select location and check in date first!");
            if (clickState !== 1) {
              setClickState(1);
            }
          } else {
            if (clickState !== 3) {
              setClickState(3);
            }
          }
        }}
      >
        <div className="relative flex justify-between items-center">
          <div>
            <h4 className="text-xs font-extrabold leading-4 pb-[2px] tracking-[0.04em]">
              Check out
            </h4>

            {endDate === null ? (
              <p className="text-sm leading-[18px] text-[#717171] font-normal overflow-hidden overflow-ellipsis">
                Add dates
              </p>
            ) : (
              <p className="text-xs font-extrabold leading-4 pb-[2px] tracking-[0.04em]">
                {format(new Date(endDate), "MMM dd")}
              </p>
            )}
          </div>
          {endDate !== null && clickState === 3 ? (
            <XCircleIcon
              className="absolute -right-5 h-5 h-5"
              onClick={() => {
                setStartDate(null);
                setEndDate(null);
                setClickState(0);
              }}
            />
          ) : null}
        </div>
      </div>
      <div className={`border-r ${hoverState === 3 ? "h-0" : "h-8"}`} />
    </>
  );
};

export default CheckOut;
